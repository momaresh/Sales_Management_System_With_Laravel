...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title"> اضافة حساب مالي جديد</h3>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <form class="form-horizantal" action="{{ route('admin.accounts.store') }}" method="post" >
                <div class="row">
                    @csrf
                                ...
                                <select name="account_type" id="account_type" class="form-control select2">
                                    ...
                                    @if (@isset($account_types) && !@empty($account_types))
                                        ...
                                        @foreach ($account_types as $info )
                                            <option @if (old('account_type') == $info->id) selected @endif value="{{ $info->id }}"> {{ $info->name }} </option>
                                        @endforeach
                            ...
                            <select name="parent_account_number" id="parent_account_number" class="form-control ">
                                ...
                                @if (@isset($parent_accounts) && !@empty($parent_accounts))
                                    ...
                                    @foreach ($parent_accounts as $info )
                                        <option @if (old('parent_account_number') == $info->account_number) selected @endif value="{{ $info->account_number }}"> {{ $info->notes }} </option>
                                    @endforeach
