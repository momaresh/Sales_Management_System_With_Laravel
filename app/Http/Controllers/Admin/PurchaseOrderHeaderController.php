...
class PurchaseOrderHeaderController extends Controller
{
    ...
    {
        //
        if (check_control_menu_role('الحركات المخزنية', 'فواتير المشتريات' , 'عرض') == true) {
            try {
    ...
    public function create()
    {
        ...
        if (check_control_menu_role('الحركات المخزنية', 'فواتير المشتريات' , 'اضافة') == true) {
            ...
            $suppliers_code = Person::where(['person_type' => 2, 'com_code' => auth()->user()->com_code, 'active' => 1])->get(['id', 'first_name', 'last_name']);
            $stores = Store::where(['com_code' => $com_code, 'active' => 1])->get(['id', 'name']);
    ...
    public function edit($id)
    {
        if (check_control_menu_role('الحركات المخزنية', 'فواتير المشتريات' , 'تعديل') == true) {
            $com_code = auth()->user()->com_code;
            $data = InvoiceOrderHeader::where(['id' => $id, 'com_code' => $com_code])->get()->first();

            if (empty($data)) {
                return redirect()->back()->with('error', 'لا يوجد بيانات كهذه متاح');
            }
            if ($data['is_approved'] == 1) {
                return redirect()->back()->with('error', 'لا يمكن تعديل فاتورة معتمدة');
            }
    ...
    {
        # code...
        if (check_control_menu_role('الحركات المخزنية', 'فواتير المشتريات' , 'اعتماد') == true) {
            if ($request->ajax()) {
                try {
                    $com_code = auth()->user()->com_code;
