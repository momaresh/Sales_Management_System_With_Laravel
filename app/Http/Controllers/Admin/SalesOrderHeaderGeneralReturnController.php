...
class SalesOrderHeaderGeneralReturnController extends Controller
{
    ...
    public function index()
    {
        ...
        if (check_control_menu_role('المبيعات', 'فواتير المرتجعات العام' , 'عرض') == true) {
            try {
                ...
                return view('admin.sales_order_header_general_return.index', ['data' => $data, 'customers' => $customers, 'delegates' => $delegates]);
            }
            catch (Exception $e) {
    ...
    public function get_item_batch(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (!empty($item_card_data)) {
                if (empty($store_id)) {
                    $item_card_batches = InvItemCardBatch::where(['item_code' => $item_code, 'com_code' => $com_code])->orderBy('id', 'DESC')->get();
                }
                else {
                    $item_card_batches = InvItemCardBatch::where(['item_code' => $item_code, 'store_id' => $store_id, 'com_code' => $com_code])->orderBy('id', 'DESC')->get();
                }
                /////////////////////////////////////////////
                ...
                if ($flag) {
                    ...
                    if ($request->what_paid > 0) {
                        ...
                        if ($data['customer_code'] == '') {
                            ...
                            $insertTransaction['byan'] = ' صرف نظير مرتجع مبيعات للعميل' . ' ' . $data['customer_name'] . ' ' . 'فاتورة رقم ' . $auto_serial;
                    ...
                    if ($data['delegate_code'] != '' && $data['delegate_code'] != null) {
                        ...
                        $insertTransaction['treasuries_id'] = $request->treasuries_id;
                        $insertTransaction['move_date'] = date('Y-m-d');
                        $insertTransaction['byan'] = ' تحصيل عمولة مرتجع مبيعات من المندوب' . ' ' . $data['delegate_name'] . ' ' . 'فاتورة رقم ' . $auto_serial;
                        $insertTransaction['is_account'] = 1;
                        $insertTransaction['money_for_account'] = $updateSalesInvoice['money_for_delegate'] * (1);
                        $insertTransaction['added_by'] = auth()->user()->id;
                        $insertTransaction['com_code'] = $com_code;
                        $insertTransaction['created_at'] = date('Y-m-d H:i:s');
                    ...
                    if (!empty($item_cards)) {
                        foreach ($item_cards as $item) {
                            ...
                            if (!empty($item_card_data)) {
                                ...
                                $insertItemMovement['byan'] = 'صرف نضير مرتجع مبيعات للعميل ' . $data['customer_name'] . ' فاتورة رقم ' . $data['pill_code'];
