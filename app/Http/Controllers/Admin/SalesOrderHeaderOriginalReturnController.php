...
class SalesOrderHeaderOriginalReturnController extends Controller
{
    ...
    public function index()
    {
        ...
        if (check_control_menu_role('المبيعات', 'فواتير المرتجعات بالاصل' , 'عرض') == true) {
            try {
                $invoice_in = InvoiceOrderHeader::where(['com_code' => auth()->user()->com_code, 'invoice_type' => 2, 'order_type' => 1])->orderBy('id' , 'desc')->get('id');
                ...
                if (!empty($data)) {
                    foreach ($data as $d) {
                        ...
                        $sales_date = SalesOrderHeader::where(['com_code' => auth()->user()->com_code, 'invoice_id' => $d['invoice_order_id']])->get()->first();
                        $d['customer_code'] = $sales_date['customer_code'];
                        $d['delegate_code'] = $sales_date['delegate_code'];
                        $d['sales_code'] = $sales_date['sales_code'];
        ...
        if (check_control_menu_role('المبيعات', 'فواتير المرتجعات بالاصل' , 'اضافة') == true) {
            try {
                ...
                if (empty($check_shift)) {
                    return redirect()->back()->with('error', 'انت لاتملك شفت حالي لعمل اضافة');
                }
                ...
                return view('admin.sales_order_header_original_return.create', ['customers' => $customers, 'check_shift' => $check_shift]);
            }
            catch (Exception $e) {
                return redirect()->back()->with('error', $e->getMessage());
            }
            ...
            try {
                ...
                if ($flag) {
                    ...
                    if ($request->what_paid > 0) {
                        ...
                        if ($data['customer_code'] != '' && $data['customer_code'] != null) {
                            $insertTransaction['byan'] = 'صرف نظير مرتجع مبيعات من العميل' . ' ' . $data['customer_name'] . ' ' . 'فاتورة مرتجع مبيعات باصل الفاتورة رقم الاصل ' . $data['pill_code'] . ' رقم المرتجع ' . $insertReturnInvoice['pill_code'];
                            $insertTransaction['is_account'] = 1;
                            $insertTransaction['money_for_account'] = $request->what_paid * (1);
                        }
                        else {
                            $insertTransaction['byan'] = 'صرف نظير مرتجع مبيعات بدون عميل' . ' ' . 'فاتورة مرتجع مبيعات باصل الفاتورة رقم الاصل ' . $data['pill_code'] . ' رقم المرتجع ' . $insertReturnInvoice['pill_code'];
                    ...
                    if ($request->what_remain > 0) {
                        ...
                        $insertTransaction['move_date'] = date('Y-m-d');
                        $insertTransaction['byan'] = ' صرف نظير مرتجع مبيعات للعميل' . ' ' . $data['customer_name'] . ' ' . 'فاتورة مرتجع مبيعات باصل الفاتورة رقم الاصل ' . $data['pill_code'] . ' رقم المرتجع ' . $insertReturnInvoice['pill_code'];
                        $insertTransaction['is_account'] = 1;
                    ...
                    if ($data['delegate_code'] != '' && $data['delegate_code'] != null) {
                        ...
                        $insertTransaction['treasuries_id'] = $request->treasuries_id;
                        $insertTransaction['move_date'] = date('Y-m-d');
                        $insertTransaction['byan'] = ' تحصيل عمولة مرتجع مبيعات من المندوب' . ' ' . $data['delegate_name'] . ' ' . 'فاتورة مرتجع مبيعات باصل الفاتورة رقم الاصل ' . $data['pill_code'] . ' رقم المرتجع ' . $insertReturnInvoice['pill_code'];
                        $insertTransaction['is_account'] = 1;
                    ...
                    foreach ($request->id as $detail_id) {
                        ...
                        if ($request->rejected_quantity[$i] > 0) {
                            ...
                            if (!empty($item_card_data)) {
                                ...
                                $insertItemMovement['byan'] = 'مرتجع مبيعات بأصل الفاتورة للعميل ' . $data['customer_name'] . ' ' . ' رقم الاصل ' . $data['pill_code'] . ' رقم المرتجع ' . $insertReturnInvoice['pill_code'];
            ...
            try {
                ...
                if (!empty($details)) {
                    foreach($details as $detail) {
                        ...
                        $detail['item_card_name'] = InvItemCard::where('item_code', $parent_details['item_code'])->value('name');
                        $detail['unit_name'] = InvUnit::where(['id' => $parent_details['unit_id'], 'com_code' => $com_code])->value('name');
                        $detail['store_name'] = Store::where(['id' => $parent_details['store_id'], 'com_code' => $com_code])->value('name');
                    }
                }
    ...
    public function ajax_search(Request $request) {
        try {
            if ($request->ajax()) {
                ...
                $sales_in = SalesOrderHeader::where("$filed2", "$operator2", "$value2")->where("$filed3", "$operator3", "$value3")->where(['com_code' => auth()->user()->com_code])->get('invoice_id');
                $invoice_in = InvoiceOrderHeader::whereIn('id', $sales_in)->where("$filed1", "$operator1", "$value1")->where(['com_code' => auth()->user()->com_code, 'invoice_type' => 2, 'order_type' => 1, 'is_original_return' => 1])->orderBy('id', 'Desc')->get('id');
    ...
    public function printA4($id, $pill_code, $type) {
        if (check_control_menu_role('المبيعات', 'فواتير المرتجعات بالاصل' , 'طباعة') == true) {
            ...
            try {
                ...
                $data['total_after_tax'] = $data['total_before_discount'] + $data['tax_value'];
                $data['discount_percent'] = round($data['discount_value'] / $data['total_after_tax'], 3);
                $data['customer_code'] = $sales_data['customer_code'];
                $data['delegate_code'] = $sales_data['delegate_code'];

                if ($data['customer_code'] != null) {
                    $person_id = Customer::where(['customer_code' => $data['customer_code'], 'com_code' => $com_code])->value('person_id');
                    $customer = Person::where(['id' => $person_id, 'com_code' => $com_code])->select(['first_name', 'last_name', 'phone'])->first();
                    $data['customer_name'] = $customer->first_name . ' ' . $customer->last_name;
