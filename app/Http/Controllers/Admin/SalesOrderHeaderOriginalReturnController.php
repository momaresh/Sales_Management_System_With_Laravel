...
class SalesOrderHeaderOriginalReturnController extends Controller
{
    ...
    public function create()
    {
        ...
        if (check_control_menu_role('المبيعات', 'فواتير المرتجعات بالاصل' , 'اضافة') == true) {
            try {
                ...
                return view('admin.sales_order_header_original_return.create', ['items_card' => $items_card, 'customers' => $customers, 'stores' => $stores, 'check_shift' => $check_shift]);
            }
            catch (Exception $e) {
                return redirect()->back()->with('error', $e->getMessage());
            }
    ...
    public function get_pill_details(Request $request)
    {
        if ($request->ajax()) {
            ...
            return view("admin.sales_order_header_original_return.get_pill_details", ['pill' => $pill, 'pill_details' => $pill_details]);
        }
    }
            ...
            try {
                ...
                if ($flag) {
                    ...
                    if ($request->total_pill > 0) {
                        ...
                        if ($data['customer_code'] != '' && $data['customer_code'] != null) {
                            $insertTransaction['byan'] = 'صرف نظير مرتجع مبيعات من العميل' . ' ' . $data['customer_name'];
